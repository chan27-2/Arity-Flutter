...
public class GraphView extends View implements
    ...
    private void init(Context context) {
        // Set colors manually
        COL_AXIS = 0xFF000000; // Black
        COL_GRID = 0xFF808080; // Gray
        COL_TEXT = 0xFF000000; // Black
        COL_BACK = 0xFFFFFFFF; // White

        // Set graph colors manually
        COL_GRAPH[0] = 0xFF4285F4; // Blue
        COL_GRAPH[1] = 0xFFDB4437; // Red
        COL_GRAPH[2] = 0xFFF4B400; // Yellow
        COL_GRAPH[3] = 0xFF0F9D58; // Green
        COL_GRAPH[4] = 0xFF9E9E9E; // Gray

        CalculatorView.log(
                "COL_AXIS " + COL_AXIS + " COL_GRID " + COL_GRID + " COL_TEXT " + COL_TEXT + " COL_BACK " + COL_BACK);
        centerX = 0;
        centerY = 0;
    ...
    /*
     * private float distance(float x1, float y1, float x2, float y2, float x, float
     * y) {
     * float dx = x2 - x1;
     * float dy = y2 - y1;
     * float mx = x - x1;
    ...
    private void computeGraph(Function f, float minX, float maxX, float minY, float maxY, Data graph) {
        ...
        while (true) {
            ...
            if (!middleIsOutside) {
                 ...
                 */
                // Calculator.log("" + dx + ' ' + leftY + ' ' + middleY + ' ' + rightY + ' ' +
                // distance + ' ' + ythresh);
        ...
        // Calculator.log("graph points " + graph.size + " evals " + nEval + " time " +
        // (t2-t1));
