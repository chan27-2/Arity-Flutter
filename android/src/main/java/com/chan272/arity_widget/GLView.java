...
abstract class GLView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void onResume() {
        CalculatorView.log("onResume " + this);
        paused = false;
        if (hasSurface) {
            initGL();
        }
    ...
    public void onPause() {
        CalculatorView.log("onPause " + this);
        deinitGL();
    }
    ...
    protected void glDraw() {
        if (hasSurface && !paused) {
            ...
            if (!egl.eglSwapBuffers(display, surface)) {
                CalculatorView.log("swapBuffers error " + egl.eglGetError());
            }
            if (egl.eglGetError() == EGL11.EGL_CONTEXT_LOST) {
                CalculatorView.log("egl context lost " + this);
                paused = true;
            }
            if (mIsLooping) {
                requestDraw();
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        CalculatorView.log("surfaceCreated " + this);
    }

    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        CalculatorView.log("surfaceChanged " + format + ' ' + this);
        this.width = width;
        this.height = height;
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        CalculatorView.log("surfaceDestroyed " + this);
        hasSurface = false;
        deinitGL();
    }
    ...
    public void startLooping() {
        if (!mIsLooping) {
            CalculatorView.log("start looping");
            mIsLooping = true;
            glDraw();
        }
    }
    ...
    public void stopLooping() {
        if (mIsLooping) {
            CalculatorView.log("stop looping");
            mIsLooping = false;
        }
    }
